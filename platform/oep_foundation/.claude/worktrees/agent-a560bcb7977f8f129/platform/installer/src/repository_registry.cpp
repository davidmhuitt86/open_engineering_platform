...
RegistryResult RepositoryRegistry::remove_install(const std::string& package_id) const {
    if (package_id.empty()) {
        return {false, "cannot remove an installed package with an empty packageId"};
    }
    ...
    std::error_code error_code;
    if (!std::filesystem::exists(path, error_code)) {
        return {false, "package '" + package_id + "' is not recorded as installed"};
    }
    ...
    // Best-effort cleanup of the now-empty <packages_root_>/<packageId>/
    // directory. Failure here (e.g. some other file was left alongside
    // registry.json) is not reported as an error -- the record itself is
    // already gone, which is remove_install's actual contract.
    std::error_code cleanup_error;
    std::filesystem::remove(path.parent_path(), cleanup_error);
