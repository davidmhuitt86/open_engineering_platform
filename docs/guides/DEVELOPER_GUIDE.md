...
- `packages/package_cli` — the `oep-package` CLI (`create`/`build`/`validate`): scaffolds and builds a `.oep` package source directory into a distributable archive, reusing `packages/manifest`/`packages/package_manager`'s existing parsing and hashing rather than re-implementing them. See its own README for what it deliberately does not do (signing, uploading).
...
- `apps/publisher-portal` — the Engineering Exchange Web Application (TASK-EXC-0009): application shell, navigation, and the Marketplace Home/Search/Package Detail/Publisher Profile/Downloads/My Library/404 views. Holds no server-side state; every page talks to `exchange-api` exclusively through `packages/exchange-client`'s `ExchangeApiClient`. See `docs/guides/FRONTEND_GUIDE.md` and `docs/guides/COMPONENT_GUIDE.md`.
- `apps/exchange-admin` — still scaffold-only; Administration is explicitly out of TASK-EXC-0009's scope (WP-EXC-009.md §2).
- `packages/exchange-client` — the typed HTTP client SDK (TASK-EXC-0009) every web app and cross-repository package (`installer`, `update-service`) uses as its only path to `exchange-api`'s REST surface. Real implementation as of this task; see `docs/guides/FRONTEND_GUIDE.md` "API client" for its shape.
...
`apps/publisher-portal` (TASK-EXC-0009) is a Vite + React + TypeScript SPA — see its own `tsconfig.json`/README for why it deliberately doesn't extend the repo's Node-targeted `tsconfig.base.json`. Conventions specific to it:

- **API access**: only through `ExchangeApiClient` (`packages/exchange-client`), obtained via `useExchangeApiClient()` (`src/api/ExchangeApiClientContext.tsx`) — no component calls `fetch` itself (WP-EXC-009.md §3).
- **Data fetching**: the shared `useAsync()` hook (`src/hooks/use-async.ts`) is the one place loading/error/success state is modeled; pages branch on `state.status` and render `LoadingIndicator`/`ErrorView`/the real content.
- **Local (non-server) state**: `LibraryContext` (`src/state/LibraryContext.tsx`) is the only `localStorage`-backed state in the app — the Downloads/My Library history, since authentication (and therefore a server-side "current user") is out of scope. Every field it stores is either a real API resource id or a value already returned by a real API call — never fabricated.
- **Styling**: one global stylesheet (`src/styles/global.css`, design tokens + component classes + responsive breakpoints), no CSS-in-JS or utility framework.
- **Full guide**: `docs/guides/FRONTEND_GUIDE.md` (architecture, routing, dev server proxy) and `docs/guides/COMPONENT_GUIDE.md` (the reusable component catalog).

## Studio integration (WP-EXC-010)

`oep_studio` (a separate repository, the Flutter/Dart desktop OEP Studio application) integrates the Exchange as a native workspace, consuming `exchange-api`'s existing REST surface through its own hand-rolled Dart client — no route, service, persistence, or wire-contract change in this repository was needed for that integration. See `docs/guides/STUDIO_INTEGRATION_GUIDE.md` for the Studio-side architecture, and this repository's own `RELEASE_NOTES_RC1.md`/`VALIDATION_REPORT.md` (`docs/`) for what was validated across both repositories.
