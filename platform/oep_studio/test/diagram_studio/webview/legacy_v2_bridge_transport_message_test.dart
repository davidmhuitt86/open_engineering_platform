...
void main() {
  group('V2ModuleMovedMessage.fromJson', () {
    test('decodes id/x/y from a moduleMoved payload', () {
      final message = V2ModuleMovedMessage.fromJson({'id': 'cdi-unit', 'x': 120, 'y': 80.5});
    ...
    test('accepts integer x/y (V2 sends whole-pixel grid-snapped values)', () {
      final message = V2ModuleMovedMessage.fromJson({'id': 'battery', 'x': 0, 'y': 0});
  ...
  group('V2WireCreatedMessage.fromJson', () {
    test('decodes id/endpoints/terminals/label/color from a wireCreated payload', () {
  ...
  group('V2StatusMessage.fromJson', () {
    test('decodes a full status snapshot', () {
      final status = V2StatusMessage.fromJson({'selM': 'ecm', 'moduleCount': 12, 'wireCount': 34});
    ...
    test('decodes a snapshot with no module selected', () {
      final status = V2StatusMessage.fromJson({'selM': null, 'moduleCount': 12, 'wireCount': 34});
    ...
    test('decodes editMode true/false', () {
      expect(V2StatusMessage.fromJson({'selM': null, 'moduleCount': 0, 'wireCount': 0, 'editMode': true}).editMode,
          isTrue);
      expect(V2StatusMessage.fromJson({'selM': null, 'moduleCount': 0, 'wireCount': 0, 'editMode': false}).editMode,
          isFalse);
    });

    test('editMode defaults to null when absent from the payload (older injected script, or V2 not yet initialized)',
        () {
      final status = V2StatusMessage.fromJson({'selM': null, 'moduleCount': 0, 'wireCount': 0});
