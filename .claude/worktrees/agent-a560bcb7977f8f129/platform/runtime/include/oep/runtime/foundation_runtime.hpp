...
// Package uninstallation (WP-REP-005). Unlike RuntimeInstallResult,
// counts are only meaningful when success is true: uninstall_package
// runs inside a Repository Transaction, so a failure partway through
// rolls back every delete already performed, leaving the package (and
// every object/relationship it contributed) exactly as installed.
...
public:
    ...
    // ---------------------------------------------------------------
    //
    // uninstall_package deletes every Engineering Object and Relationship
    // the Repository Registry recorded for `package_id` (via the same
    // ObjectStore/RelationshipStore delete_object/delete_relationship
    // paths a caller-driven delete would use, so undo/journal behavior is
    // identical), then removes the Repository Registry record itself.
    // Objects are deleted after relationships, mirroring the dependency
    // order install_package creates them in reverse -- though
    // ObjectStore::remove does not cascade to Relationships (see
    // oep_object_delete's own documentation), deleting relationships
    // first avoids ever leaving a dangling Relationship referencing an
    // already-deleted Object mid-uninstall.
    //
    // ATOMIC, exactly like install_package: the whole uninstall runs
    // inside one Repository Transaction (WP-REP-003). A failure partway
    // through — a missing object/relationship (already deleted by some
    // other path), or a Repository Registry write failure — rolls back
    // every delete already performed, and the transaction is journaled
    // (RolledBack) either way. Fails outright (deletes nothing) if
    // `package_id` is not currently installed, or if called inside an
    // explicit caller transaction (an uninstall is its own transaction,
    // never a participant in someone else's — no nesting, per Work
    // Package 014).
