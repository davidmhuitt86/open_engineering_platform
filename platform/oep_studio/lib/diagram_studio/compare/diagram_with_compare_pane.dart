...
///
/// Top-level (not a `DiagramWithComparePane` instance method) —
/// PRODUCT-READINESS-012 §Analyze: the engineering toolbar's own
/// Analyze dropdown (index.html's `dd-analyze`) needs to trigger this
/// SAME real mutual-exclusion logic via the engineeringCommand bridge
/// (`_handleEngineeringCommand`, legacy_v2_webview.dart), which cannot
/// reach a private instance method on a different widget.
void deactivateOtherDiagramPanels(WidgetRef ref, {required String except}) {
...
void toggleAnalysisPanel(WidgetRef ref, bool analysisEnabled) {
  if (!analysisEnabled) deactivateOtherDiagramPanels(ref, except: 'analysis');
  ref.read(analysisPanelVisibleProvider.notifier).state = !analysisEnabled;
}

Future<void> toggleComparePane(BuildContext context, WidgetRef ref) async {
  ...
  if (!context.mounted) return;
  deactivateOtherDiagramPanels(ref, except: 'compare');
  ref.read(compareModeEnabledProvider.notifier).state = true;
}

void toggleDmmPanel(WidgetRef ref, bool dmmEnabled) {
  if (!dmmEnabled) deactivateOtherDiagramPanels(ref, except: 'dmm');
  ref.read(dmmPanelVisibleProvider.notifier).state = !dmmEnabled;
}

void toggleTracePanel(WidgetRef ref, bool traceEnabled) {
  if (!traceEnabled) deactivateOtherDiagramPanels(ref, except: 'trace');
...
class DiagramWithComparePane extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // enforced once, at the point of activation (`toggleAnalysisPanel`/
    // `toggleComparePane`/`toggleDmmPanel`/`toggleTracePanel`, all
    // above), so at
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                TextButton.icon(
                  onPressed: () => toggleAnalysisPanel(ref, analysisEnabled),
                  icon: Icon(
                      analysisEnabled
                          ? Icons.close
                ...
                TextButton.icon(
                  onPressed: () => toggleComparePane(context, ref),
                  icon: Icon(
                      compareEnabled
