...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await legacyV2PersistenceSettle(tester);
      channel.simulateWireCreated('wire-1', 'gnd-1', 'gnd-2', 'Bridging Wire', 'G', fromTerminal: 'A', toTerminal: 'B');
      await legacyV2PersistenceSettle(tester);
      ...
      final filePath = '${documentsDir.path}${Platform.pathSeparator}bridge008.json';
      await tester.runAsync(() => controller.saveDocumentAs(filePath));
      expect(controller.documentPath, filePath);
      expect(controller.isDirty, isFalse, reason: 'a successful save must clear dirty state');
      expect(File(filePath).existsSync(), isTrue, reason: 'saveDocumentAs must actually write the file');

      final onDiskRaw = File(filePath).readAsStringSync();
      expect(onDiskRaw, contains('"v2ModuleId"'), reason: 'bridge identity metadata must be present in the saved JSON');
      expect(onDiskRaw, contains('"v2WireId"'));
      expect(onDiskRaw, contains('"wireColor"'));
      expect(onDiskRaw, contains('"documentId"'), reason: 'DiagramDocument.id must be persisted in the envelope');
      ...
      expect(controller.document.id, isNot(documentIdBeforeSave),
          reason: 'newDocument() must generate a distinct id, not reuse the saved document\'s');
      expect(adapter.oepNodeIdFor('gnd-1'), originalNodeIdA,
          reason: 'the OLD adapter\'s in-memory map is untouched by a document switch -- demonstrating it is now '
      ...
      expect(controller.engine.editing.session.graph.nodes.containsKey(originalNodeIdA), isFalse,
          reason: 'the node from the saved document must not exist in the new empty document\'s graph');
      ...
      await legacyV2PersistenceSettle(tester);
      expect(controller.documentPath, filePath);
      expect(controller.isDirty, isFalse, reason: 'a freshly opened document must be clean');
      expect(controller.document.id, documentIdBeforeSave,
          reason: 'DiagramDocument.id must round-trip through save/reload unchanged');
      ...
      final reloadedNodes = controller.engine.editing.session.graph.nodes.values.toList();
      expect(reloadedNodes.length, 2);
      expect(reloadedNodes.map((n) => n.metadata['v2ModuleId']).toSet(), {'gnd-1', 'gnd-2'});
      final reloadedRelationships = controller.engine.editing.session.graph.relationships.values.toList();
      ...
      final freshAdapter = LegacyV2StateAdapter(controller: controller, channel: freshChannel);
      ...
      expect(freshChannel.restoredModuleIds, containsAll(['gnd-1', 'gnd-2']),
          reason: 'V2 must be reseeded with both modules from the reloaded document');
      expect(freshChannel.restoredWireIds, contains('wire-1'));
      final restoredWireCall = freshChannel.restoredWires.singleWhere((w) => w.$1 == 'wire-1');
      expect(restoredWireCall.$4, 'Bridging Wire', reason: 'label must survive the real disk round trip');
      expect(restoredWireCall.$5, 'G', reason: 'wireColor must survive the real disk round trip');
      final restoredModuleA = freshChannel.restoredModules.singleWhere((m) => m.$1 == 'gnd-1');
      expect(restoredModuleA.$4, 10.0, reason: 'x position must survive the real disk round trip');
      expect(restoredModuleA.$5, 20.0, reason: 'y position must survive the real disk round trip');
      ...
      expect(controller.engine.editing.session.graph.nodes.containsKey(rebuiltNodeIdA), isTrue);
      expect(controller.engine.editing.session.graph.relationships.containsKey(rebuiltRelId), isTrue);
    },
  );
}
