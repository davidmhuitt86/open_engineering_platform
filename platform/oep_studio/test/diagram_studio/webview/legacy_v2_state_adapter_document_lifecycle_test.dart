...
class _FakeChannel implements LegacyV2Channel {
  ...
  void simulateWireCreated(String v2WireId, String fromModuleId, String toModuleId) => _onWireCreated?.call(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(engine.editing.session.graph.nodes.length, beforeGate,
          reason: 'a message received before initializeFromDocument must be ignored, not queued');

      await adapter.initializeFromDocument();
      expect(adapter.isReady, isTrue);
      expect(channel.restoredModuleIds, isEmpty, reason: 'a fresh empty document has nothing to seed');
      ...
      final relJson = engine.editing.session.graph.relationships[relId]!.toJson();
      ...
      expect(freshAdapter.oepNodeIdFor('gnd-1'), nodeIdA,
          reason: 'identity must be reconstructible from document metadata alone');
