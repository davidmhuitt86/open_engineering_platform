...
Establishes the minimal Foundation-side concept required for a Studio (e.g. OEP Studio's Diagram Studio) to commit an Engineering Graph and later load back **only that graph's** own objects and relationships, rather than the entire open repository. Per `oep_foundation/CLAUDE.md`'s Five Primitive Rule, this is **not** a new primitive: a diagram/graph identity is an ordinary `ObjectType::Diagram` Engineering Object (the type already existed), and membership is a single additive `diagram_id` string field on the existing `EngineeringObject`/`Relationship` primitives, empty for anything that belongs to no diagram. Deliberately named `diagram_id`, never `graph_id`, to avoid colliding with the pre-existing, unrelated, never-persisted whole-repository traversal concept already called a "graph" elsewhere in Foundation (`oep::repository::GraphEngine`) and with `oep_engine`'s separate, larger Knowledge Graph (OEP-SPEC-007).

```c
oep_object_info_t diagram;
oep_diagram_create(runtime, "Ignition Circuit", "Primary ignition wiring diagram", "engineer@example.com", &diagram);

oep_object_info_t battery;
oep_object_create_with_diagram(runtime, OEP_OBJECT_TYPE_COMPONENT, "Battery", "12V lead-acid", "engineer@example.com",
                                NULL, 0, diagram.object_id, &battery);

oep_relationship_info_t wire;
oep_relationship_create_with_diagram(runtime, battery.object_id, other.object_id, OEP_RELATIONSHIP_TYPE_CONNECTED_TO,
                                      "engineer@example.com", "12AWG", diagram.object_id, &wire);

oep_object_list_t members; oep_relationship_list_t links;
oep_diagram_get_objects(runtime, diagram.object_id, &members);
oep_diagram_get_relationships(runtime, diagram.object_id, &links);
/* members/links contain ONLY this diagram's own contents, never another diagram's */
oep_object_list_release(&members);
oep_relationship_list_release(&links);
```

- `oep_diagram_create` — creates a new `ObjectType::Diagram` Engineering Object via `ObjectStore::create` (unchanged, existing identity mechanism — a normal UUID object id) and returns it as the diagram's persistent identity. No new store, no new file format.
- `oep_diagram_get` — resolves a diagram identity by id; fails with `OEP_ERROR_NOT_FOUND` for an unknown id or an id that names an object that isn't a Diagram.
- `oep_object_create_with_diagram`/`oep_relationship_create_with_diagram` — identical to `oep_object_create`/`oep_relationship_create` except for one additional `diagram_id` parameter, validated against `oep_diagram_get` before the member is created; an invalid `diagram_id` fails with `OEP_ERROR_INVALID_ARGUMENT` and creates nothing. The member's own `object_id`/`relationship_id` is assigned exactly as it always was — diagram membership never replaces or wraps existing object/relationship identity.
- `oep_diagram_get_objects`/`oep_diagram_get_relationships` — enumerate exactly the objects/relationships whose `diagram_id` matches, sorted deterministically by id (ascending, byte-wise), the same convention `oep_object_store_list`/`oep_relationship_store_list` already established. An empty diagram returns a successful, zero-length list — not an error, not indistinguishable from an invalid id (an invalid `diagram_id` itself fails with `OEP_ERROR_INVALID_ARGUMENT`).
- `oep_object_info_t`/`oep_relationship_info_t` both gained a `diagram_id` field (appended at the end of each struct, per this header's struct-extension convention) — plain object/relationship creation (`oep_object_create`/`oep_relationship_create`) leaves it empty, unchanged existing behavior.

**Atomicity:** diagram creation and member creation both go through the same implicit-transaction machinery every other mutation already uses (WP-REP-003) — a diagram created inside an explicit `oep_transaction_begin`/`_rollback` disappears completely on rollback, along with any members created alongside it in the same transaction.

**Not addressed by this work package:** the Engine/Studio-side Dart FFI bindings, `FoundationBridge` wrapper methods, and `StudioFoundationBridgePort.loadCommittedGraph()` scoping remain unimplemented — these six C functions are exported (`oep_foundation_bridge.def`) and ABI-stable but not yet callable from Dart. Wiring them in is the next package.
