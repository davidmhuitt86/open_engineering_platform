...
TEST_CASE("V7 migration applies cleanly and produces the expected artifact_metadata schema",
  ...
  const auto columns =
      txn.exec("SELECT column_name FROM information_schema.columns "
               "WHERE table_name = 'artifact_metadata' ORDER BY ordinal_position");
  ...
  const std::vector<std::string> expected = {
      "id",         "uuid",          "verification_id", "file_name",       "file_extension",
      "mime_type",  "file_size_bytes", "sha256_hash",    "file_created_at", "file_modified_at",
      "pdf_version", "pdf_page_count", "status",         "extracted_at",   "error_message",
      "created_at", "updated_at"};
  CHECK(column_names == expected);

  const auto extracted_at_nullable =
      txn.exec("SELECT is_nullable FROM information_schema.columns "
               "WHERE table_name = 'artifact_metadata' AND column_name = 'extracted_at'");
  REQUIRE(extracted_at_nullable.size() == 1);
  CHECK(extracted_at_nullable[0][0].as<std::string>() == "YES");

  const auto status_not_nullable =
      txn.exec("SELECT is_nullable FROM information_schema.columns "
               "WHERE table_name = 'artifact_metadata' AND column_name = 'status'");
  ...
  const auto foreign_key =
      txn.exec("SELECT constraint_name FROM information_schema.table_constraints "
               "WHERE table_name = 'artifact_metadata' AND constraint_type = 'FOREIGN KEY'");
  CHECK(foreign_key.size() == 1);

  // No deleted_at column -- WORK_PACKAGE-008 has no DELETE route.
  const auto deleted_at =
      txn.exec("SELECT column_name FROM information_schema.columns "
               "WHERE table_name = 'artifact_metadata' AND column_name = 'deleted_at'");
