...
- **Applications may depend on Exchange Packages, the Core Package, and `interfaces`.** An application (`exchange-api`, `publisher-portal`, `exchange-admin`) may never be depended upon by a package — dependencies point strictly downward. `exchange-api` depends on `installer` (TASK-EXC-0008) for its `RepositoryClient` implementations, the same way it depends on `manifest`/`package-manager`/`search`.
- **Exchange Packages may depend on the Core Package and on each other**, as long as the resulting graph is acyclic (see §3). Today's real inter-package edges: `package-manager` → `manifest` + `signing`; `exchange-client` → `api-contracts`; `installer` → `exchange-client` + `interfaces`; `update-service` → `exchange-client`; `dependency-resolver` → `manifest`; `reviews`/`search` → `api-contracts`. See `docs/architecture/REPOSITORY_STRUCTURE.md` §2 for the full current graph.
...
- **`interfaces` depends on nothing.** `installer`'s dependency on it (TASK-EXC-0008, `RepositoryClient`) is the only edge into this package that exists today.
- **Only `installer` depends on `interfaces`** — no other package has a legitimate reason to reach toward a cross-repository contract.
...
- **`oep_foundation` / `oep_repository`** — the `RepositoryClient` contract (TASK-EXC-0008, `packages/interfaces/src/repository-client.ts`), invoked by `installer`'s `HttpRepositoryClient` to perform the Package Transaction Engine → Repository Merge Engine → Repository Validation sequence (WP-EXC-001 §6 "Repository Integration") through Foundation's public interfaces only — the first of these integration points to actually be defined. No `oep_repository` codebase exists anywhere in the platform yet and `oep_foundation`'s own Repository documents a Public **C API**, not REST; `HttpRepositoryClient` is a real, working HTTP client against WP-EXC-008.md's own anticipated "Repository Public API" shape, not a dependency on any concrete Repository implementation (which remains explicitly out of `oep_exchange`'s scope). `apps/exchange-api` defaults to `StubRepositoryClient` until a real Repository is reachable — see `docs/architecture/REPOSITORY_STRUCTURE.md` §18.1.
