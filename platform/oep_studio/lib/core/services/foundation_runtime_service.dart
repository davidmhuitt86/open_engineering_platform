...
class FoundationRuntimeNotifier extends Notifier<FoundationServiceState> {
    ...
    for (final region in state.evidenceRegions) {
      if (region.id == regionId) {
        ...
        updated = region.copyWith(
          notes: trimmed,
          modifiedTime: DateTime.now(),
          // Keep an existing Observation's description in step with the
          // notes field the UI edits (INGEST-012).
          annotation: region.annotation?.copyWith(description: trimmed),
        );
        regions.add(updated);
      } else {
        regions.add(region);
  ...
  // -------------------------------------------------------------------
  // Annotation properties (WP-INGEST-013)
  //
  // Edit the open-ended `EvidenceAnnotation.properties` of an existing
  // Evidence Region. Each edit updates that ONE region in place and
  // persists through the same `_persistActiveSession` path as every other
  // region mutation; none of them creates a region, creates or touches a
  // Knowledge Candidate, creates an Engineering Object, or commits. The
  // annotation's `id` is minted the first time a region gets one and is
  // never regenerated by a later edit.
  // -------------------------------------------------------------------

  /// Appends a new, empty text property to [regionId]'s annotation
  /// (creating the annotation payload on first use).
  void addAnnotationProperty(String regionId, {AnnotationProperty? property}) {
    _mutateAnnotation(
      regionId,
      (annotation) => annotation.withPropertyAdded(property ?? const AnnotationProperty(key: '')),
    );
  }

  /// Replaces the property at [index]. The key is normalized to its stable
  /// machine-readable form ([AnnotationProperty.normalizeKey]).
  void updateAnnotationProperty(String regionId, int index, AnnotationProperty property) {
    _mutateAnnotation(
      regionId,
      (annotation) => annotation.withPropertyUpdated(
        index,
        property.copyWith(key: AnnotationProperty.normalizeKey(property.key)),
      ),
    );
  }

  /// Removes the property at [index]; an empty list is a valid state.
  void removeAnnotationProperty(String regionId, int index) {
    _mutateAnnotation(regionId, (annotation) => annotation.withPropertyRemoved(index));
  }

  /// INGEST-012: a fresh Observation for [region], seeded once with its
  /// provenance (region id, origin, annotator, status) and the region's
  /// notes as its description. Never touches candidates or the Repository.
  EvidenceAnnotation _newObservationFor(EvidenceRegion region) => EvidenceAnnotation(
        id: KnowledgeSessionService.generateId('annotation'),
        regionId: region.id,
        origin: region.origin,
        authorId: region.annotatorId,
        status: region.status,
        description: region.notes.isEmpty ? null : region.notes,
      );

  /// INGEST-012: records the classification (type + name) on the region's
  /// Observation, creating it on first use. Called by
  /// `applyRegionClassification`; the Observation's type/name are mirrored
  ...
  void _mutateAnnotation(String regionId, EvidenceAnnotation Function(EvidenceAnnotation) change) {
    ...
    for (final region in state.evidenceRegions) {
      if (region.id == regionId) {
        final current = region.annotation ?? _newObservationFor(region);
        updated = region.copyWith(annotation: change(current), modifiedTime: DateTime.now());
        regions.add(updated);
      } else {
        regions.add(region);
      }
